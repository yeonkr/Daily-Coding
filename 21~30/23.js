...
const bubbleSort = function(arr) {
  let count = 0;   // 자리를 바꿀 필요가 없을 경우 계산 
  for (let i = 0; i < arr.length - 1; i++) {
    for (let j = 0; j < arr.length - 1 - i; j++) { // 큰 수부터 정렬되므로 이미 정렬한 숫자는 제외
      if (arr[j] > arr[j + 1]) { // 현재 요소의 값과 다음 요소의 값을 비교 --> 큰 값을 다음으로 보냄 
        count = arr[j]
        arr[j] = arr[j + 1]
        arr[j + 1] = count
      }
    } 
    if (count === 0) return arr; // 0이라면 정렬이 이미 된 상태이므로 실행 종료
  } return arr;
};